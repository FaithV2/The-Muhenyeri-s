...
function App() {
  ...
  return <>
    ...
    {opened && <main className="site">
      ...
      <section className="dress-code section"><div className="section-label">03 <span /> Guest colours</div><div className="dress-code-grid"><div><p className="eyebrow">A note for our guests</p><h2>Come dressed<br /><i>in celebration.</i></h2><p className="body-copy">{wedding.dressCode.attire}</p><p className="body-copy">For the White wedding celebration wear strictly African Opulence · shades of brown.</p><p className="dress-note">{wedding.dressCode.note}</p></div><div className="palette"><p className="eyebrow">{wedding.dressCode.title}</p><div className="swatches">{wedding.dressCode.colors.map((color) => <div className="swatch" key={color.name}><span style={{ backgroundColor: color.value }} /><p>{color.name}</p></div>)}</div></div></div></section>
        <section className="story section" id="story"><div className="section-label">04 <span /> Our story</div><div className="story-grid"><div className="story-heading"><p className="eyebrow">It started with Shalom</p><h2>Two paths,<br /><i>one journey.</i></h2></div><div className="story-copy"><p>Some stories are written in grand gestures. Ours grew in the little moments: long walks, shared playlists, and the comfortable quiet of simply being together.</p><p>Now, surrounded by our favourite people, we are ready for the next beautiful chapter.</p><span className="story-initials">M <b>+</b> S</span></div></div><div className="gallery"><PhotoFrame className="story-portrait story-maxwell" src={maxwellImage} alt="Maxwell" /><PhotoFrame className="story-ring" src={ringsImage} alt="Wedding rings" fit="contain" /><PhotoFrame className="story-portrait story-shirley" src={shirleyImage} alt="Shirley" /></div></section>
